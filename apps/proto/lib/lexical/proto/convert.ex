defmodule Lexical.Proto.Convert do
  ...
  def to_lsp(%_{result: result} = response) do
    ...
    case Convertable.to_lsp(result, context_document) do
      {:ok, converted} ->
  ...
  def to_lsp(other) do
    ...
    Convertable.to_lsp(other, context_document)
  end
  ...
  def to_native(%{lsp: request_or_notification} = original_request) do
    ...
    with {:ok, native_request} <- Convertable.to_native(request_or_notification, context_document) do
      updated_request =

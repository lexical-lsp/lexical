defmodule Lexical.RemoteControl.CodeMod.ExtractFunction do
  ...
  def extract_function(%Z{} = zipper, start_line, end_line, function_name)
    ...
    if Enum.empty?(acc.lines) do
      ...
      new_function_zipper = Z.zip(new_function(function_name, [], acc.lines))
      declared_vars = Enum.uniq(vars_declared(new_function_zipper))
      used_vars = Enum.uniq(vars_used(new_function_zipper))

defmodule Lexical.RemoteControl.Completion do
  ...
  """
  def expand(code) when is_binary(code) do
    code
    |> String.to_charlist()
    |> Enum.reverse()
    |> expand()
  ...
  defp expand_code(code) when is_list(code) do
    code = Enum.reverse(code)
    helper = get_helper(code)

    case Code.Fragment.cursor_context(code) do
      {:alias, alias} ->
        expand_aliases(List.to_string(alias))

      {:unquoted_atom, unquoted_atom} ->
        expand_erlang_modules(List.to_string(unquoted_atom))

      expansion when helper == ?b ->
        expand_typespecs(expansion, &get_module_callbacks/1)

      expansion when helper == ?t ->
        expand_typespecs(expansion, &get_module_types/1)

      {:dot, path, hint} ->
        expand_dot(path, List.to_string(hint), false)

      {:dot_arity, path, hint} ->
        expand_dot(path, List.to_string(hint), true)

      {:dot_call, path, hint} ->
        expand_dot_call(path, List.to_atom(hint))

      :expr ->
        expand_container_context(code, :expr, "") || expand_local_or_var("")

      {:local_or_var, local_or_var} ->
        hint = List.to_string(local_or_var)
        expand_container_context(code, :expr, hint) || expand_local_or_var(hint)

      {:local_arity, local} ->
        expand_local(List.to_string(local), true)

      {:local_call, local} ->
        expand_local_call(List.to_atom(local))

      {:operator, operator} when operator in ~w(:: -)c ->
        expand_container_context(code, :operator, "") ||
          expand_local(List.to_string(operator), false)

      {:operator, operator} ->
        expand_local(List.to_string(operator), false)

      {:operator_arity, operator} ->
        expand_local(List.to_string(operator), true)
      ...
      {:struct, struct} when is_list(struct) ->
        expand_structs(List.to_string(struct))
  ...
  defp expand_typespecs({:dot, path, hint}, fun) do
  ...
  defp expand_typespecs(_, _), do: no()

  ## Expand call

  defp expand_local_call(fun) do
    imports_from_env()
  ...
  defp expand_dot_call(path, fun) do
    case expand_dot_path(path) do
      {:ok, mod} when is_atom(mod) -> get_signatures(fun, mod) |> expand_signatures()
      _ -> no()
    end
  end
  ...
  defp expand_signatures([_ | _] = signatures) do
    [head | tail] = Enum.sort(signatures, &(String.length(&1) <= String.length(&2)))
    if tail != [], do: IO.write("\n" <> (tail |> Enum.reverse() |> Enum.join("\n")))
    yes("", [head])
  end

  defp expand_signatures([]), do: expand_local_or_var("")

  ## Expand dot

  defp expand_dot(path, hint, exact?) do
    case expand_dot_path(path) do
  ...
  defp expand_dot_path({:unquoted_atom, var}) do
    {:ok, List.to_atom(var)}
  end

  defp expand_dot_path(path) do
    case recur_expand_dot_path(path) do
      {:ok, [_ | _] = path} -> value_from_binding(Enum.reverse(path))
      other -> other
    end
  end

  defp recur_expand_dot_path({:var, var}) do
    {:ok, [List.to_atom(var)]}
  end

  defp recur_expand_dot_path({:alias, var}) do
    {:ok, var |> List.to_string() |> String.split(".") |> value_from_alias()}
  end

  defp recur_expand_dot_path({:dot, parent, call}) do
    case recur_expand_dot_path(parent) do
      {:ok, [_ | _] = path} -> {:ok, [List.to_atom(call) | path]}
      _ -> :error
    end
  end

  defp recur_expand_dot_path(_) do
    :error
  end
  ...
  defp expand_local_or_var(hint) do
    format_expansion(match_var(hint) ++ match_local(hint, false), hint)
  end

  defp expand_local(hint, exact?) do
    format_expansion(match_local(hint, exact?), hint)
  end

  defp expand_sigil() do
    sigils =
      match_local("sigil_", false)
      |> Enum.map(fn %{name: "sigil_" <> rest} -> %{kind: :sigil, name: rest} end)

    format_expansion(match_local("~", false) ++ sigils, "~")
  end

  defp match_local(hint, exact?) do
    imports = imports_from_env() |> Enum.flat_map(&elem(&1, 1))
    module_funs = get_module_funs(Kernel.SpecialForms)
    match_module_funs(imports ++ module_funs, hint, exact?)
  end

  defp match_var(hint) do
    variables_from_binding(hint)
    |> Enum.sort()
  ...
  defp expand_structs(hint) do
    aliases =
      for {alias, mod} <- aliases_from_env(),
  ...
  defp expand_container_context(code, context, hint) do
        ...
        case Macro.path(quoted, &match?({:__cursor__, _, []}, &1)) do
          [cursor, {:%{}, _, pairs}, {:%, _, [{:__aliases__, _, aliases}, _map]} | _] ->
            with {pairs, [^cursor]} <- Enum.split(pairs, -1),
                 alias = value_from_alias(aliases),
  ...
  defp expand_aliases(all) do
    case String.split(all, ".") do
      [hint] ->
        all = match_aliases(hint) ++ match_elixir_modules(Elixir, hint)
      ...
      parts ->
        ...
        value_from_alias(list)
        |> match_elixir_modules(hint)
        |> format_expansion(hint)
    end
  end

  defp value_from_alias([name | rest]) do
  ...
  defp match_aliases(hint) do
    for {alias, module} <- aliases_from_env(),
        [name] = Module.split(alias),
  ...
  defp imports_from_env() do
    []
    # with {evaluator, server} <- IEx.Broker.evaluator(),
